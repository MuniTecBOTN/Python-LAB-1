...
# ------------------ FRAME TÍTULO ------------------
frame_titulo = CTkFrame(master=frame_principal, height=80, fg_color="#3d3161", corner_radius=0)
...
# ------------------ FUNCIONES ---------------
# get() es el metodo para obtener el valor de las entradas
...
# sin espcios al inicio o al final, y en minusculas (estandarizar el formato del dato)
...
def capturar_datos():
    ...
    if not nombre.replace(" ","").isalpha():
        etiqueta_informacion.configure(text="el nombre no puede contener numeros")
        etiqueta_informacion.after(2000, lambda: etiqueta_informacion.configure(text=""))
        campo_nombre.configure(border_color ="#bd2249")
        campo_nombre.after(3000, lambda:campo_nombre.configure(border_color = "#e3e5f3"))
        return
    ...
    if not correo.endswith(tuple(lista_extensiones_validas)):
        etiqueta_informacion.configure(text="por favor, ingrese correo valido")
        etiqueta_informacion.after(2000, lambda: etiqueta_informacion.configure(text=""))
        campo_correo.configure(border_color ="#bd2249")
        campo_correo.after(3000, lambda:campo_correo.configure(border_color = "#e3e5f3"))
        return
...
etiqueta_genero.grid(row=5, column=0, sticky="e")
